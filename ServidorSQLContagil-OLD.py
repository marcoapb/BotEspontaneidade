...
def trataMsgRecebida(msgRecebida, c): #c é o socket estabelecido com o cliente que será utilizado para a resposta
    ...
    if 2<=codigo<=5 or codigo in [14, 16, 17]: #verificações COMUNS relativas ao TDPF
        ...
        comando = "Select Encerramento, Nome from TDPFS Where Numero=%s"        
        ...
        if row:
            ...
            logging.info("a"+nome+"z")
            logging.info(str(len(nome)))
            if row[0]!=None:
                msg = "TDPF encerrado"
                msg = msg.ljust(tamMsg)
                resposta = codigoStr+"N"+msg+nome
                enviaResposta(resposta, c) 
                conn.close()
                return               
        if not achou: 
    ...
    if codigo==17: #altera atividade de um tdpf
        ...
        atividade = msgRecebida[-83:-33].strip()     
        if len(atividade)<4:
            resposta = "99REQUISIÇÃO INVÁLIDA - ATIVIDADE - DESCRIÇÃO CURTA (18E)"
            enviaResposta(resposta, c) 
            conn.close()
            return                  
        inicio = msgRecebida[-33:-23]
        try:
            ...
            resposta = "99REQUISIÇÃO INVÁLIDA - DATA DE INÍCIO INVÁLIDA (18F)"
            enviaResposta(resposta, c) 
            conn.close()
            return
        if inicio>datetime.now():  
            resposta = "99REQUISIÇÃO INVÁLIDA - DATA DE INÍCIO FUTURA (18G)"
            enviaResposta(resposta, c) 
            conn.close()
            return                                  
        vencimento = msgRecebida[-23:-13]
        try:
            ...
            resposta = "99REQUISIÇÃO INVÁLIDA - DATA DE VENCIMENTO INVÁLIDA (18H)"
            enviaResposta(resposta, c) 
            conn.close()
            return        
        if inicio>vencimento or vencimento.date()<datetime.now().date():
            resposta = "99REQUISIÇÃO INVÁLIDA - DATA DE VENCIMENTO ANTERIOR À DE INÍCIO OU PASSADA (18I)"
            enviaResposta(resposta, c) 
            conn.close()
            return 
        termino = msgRecebida[-13:-3]
        if termino!="00/00/0000":
            try:
                ...
                resposta = "99REQUISIÇÃO INVÁLIDA - DATA DE TÉRMINO INVÁLIDA (18J)"
                enviaResposta(resposta, c) 
                conn.close()
                return        
            if inicio>terminoAux or terminoAux>datetime.now():
                resposta = "99REQUISIÇÃO INVÁLIDA - DATA DE TÉRMINO ANTERIOR À DE INÍCIO OU FUTURA (18K)"
                enviaResposta(resposta, c) 
                conn.close()
                return
        else:
        ...
        if not horas.isdigit() and horas!="AAA":
            resposta = "99REQUISIÇÃO INVÁLIDA - HORAS INVÁLIDAS (18L)"
            enviaResposta(resposta, c) 
            conn.close()
            return
        try:
            ...
            resposta = "99REQUISIÇÃO INVÁLIDA - HORAS INVÁLIDAS (18M)"
            enviaResposta(resposta, c) 
            conn.close()
            return  
        if horas<0:
            resposta = "99REQUISIÇÃO INVÁLIDA - HORAS INVÁLIDAS (18N)"
            enviaResposta(resposta, c) 
            conn.close()
            return                                  
        #verificamos se o tdpf existe e se é de responsabilidade do usuário e está em andamento
        ...
        try:
            ...
            conn.commit()
            resposta = "18ALTERAÇÃO EFETIVADA"
        except:
            conn.rollback()
            resposta = "18ERRO NA ALTERAÇÃO"
        enviaResposta(resposta, c)  
        conn.close()
        return             
